<template>
  <div class="glass-card">
    ...
    <div class="action-buttons" v-if="selectedSeatIds.length > 0">
      <button class="glow-button" @click="confirmSelection">确认选座</button>
...
import { getSeatsByHallId } from '@/services/seat'; // 请确保实现此服务函数
...
const confirmSelection = () => {
  ...
  const seat_numbers = selectedSeats.map(seat => seat.seatNumber).join(',');
  const seat_count = selectedSeatIds.value.length; // 新增：座位数量
  router.push({
    name: 'CreateOrder',
    query: {
      id,
